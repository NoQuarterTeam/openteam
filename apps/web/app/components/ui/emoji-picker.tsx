...
function EmojiPickerFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      className={cn("flex w-full min-w-0 max-w-(--frimousse-viewport-width) items-center gap-1 border-t p-2", className)}
      ...
      <EmojiPickerPrimitive.ActiveEmoji>
        {({ emoji }) =>
          emoji ? (
            ...
            <span className="ml-1.5 flex h-7 items-center truncate text-muted-foreground text-xs">Select an emoji…</span>
          )
        }
      </EmojiPickerPrimitive.ActiveEmoji>
    </div>
